...
function ShopViewPetHome({ imgUrl }) {
  ...
  useEffect(() => {
    ...
    axiosInstance.post(`/shop/viewPetHomesByShopId/${id._id}`).then((res) => {
      if (res.status === 200) {
  ...
  const remove = (id) => {
    // alert("removed")
    axiosInstance
      .post(`/shop/removePetHomeById/${id}`)
      .then((res) => {
        console.log(res);
  ...
  return (
    <>
      <div className="container container-box">
        ...
        <Link to={`/petshop/add-pethome`} id='shop-add-button' ><h1>+</h1></Link>
          ...
          {data.length ? (
              ...
              return (
                <div className="col-6 box">
                  <div className="card admin-view-pets-card1 bg-secondary-subtle">
                    ...
                    <Link to={`/petshop/edit-pethome/${user._id}`}> <button className="btn btn-primary btn-sm rounded-start-pill rounded-end-pill  align-items-center ">Edit</button></Link>
            ...
            <div className="col">
              <div class="card" style={{ width: "18rem;" }}>
                <div class="card-body">
                  <h5 class="card-title">No Homes Added</h5>
                </div>
              </div>
            </div>
...
export default ShopViewPetHome